...
sensor_msgs::msg::JointState torque_state;
...
public:
    ...
    {
        client_sub_ = this->create_subscription<moveit_msgs::msg::DisplayTrajectory>(
            "/display_planned_path", 1000, std::bind(&ClientNode::callback, this, std::placeholders::_1));
        
        torque_state_publisher_ = this->create_publisher<sensor_msgs::msg::JointState>(
            "torque_state_topic", 10);

       //loop_rate_ = rclcpp::Rate(500);
    }
...
private:
    ...
    rclcpp::Subscription<moveit_msgs::msg::DisplayTrajectory>::SharedPtr client_sub_;
    rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr torque_state_publisher_;
...
void sendSimpleCanCommand(uint8_t numOfActuator, uint8_t *canIdList, int Command)
{
    ...
    auto torque_state_publisher_ = client_node->create_publisher<sensor_msgs::msg::JointState>(
            "torque_state_topic", 10);
    torque_state.header.stamp = rclcpp::Clock().now();
    torque_state.name = {"A", "B", "C", "D", "E", "F"}; 
    torque_state.position.resize(6);
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (VCI_Transmit(VCI_USBCAN2, 0, 0, send, 1) == 1)
        {
            int count = 0,reclen=0,ind=0; // 数据列表中，用来存储列表序号。
            ...
            if ((reclen = VCI_Receive(VCI_USBCAN2, 0, ind, rec, 3000, 100)) > 0) 
            {
                for (int j = 0; j < reclen; j++)
                {
                    ...
                    torque_state.position[i] = decimal; 
                }
            }
            send[0].ID += 1;
        }
...
bool init_can()
{
    ...
    int count = 0,num=0;        // 数据列表中，用来存储列表序号。
