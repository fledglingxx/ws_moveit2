...
int32_t sendCommand(uint8_t numOfActuator, uint8_t *canIdList, uint8_t *commandList, uint32_t *parameterList)
{
    ...
    *(uint32_t *)(buf + index) = calcCRC32(0, buf, index);
    ...
    int cnt=sp.write(uartTxBuf);
    std::cout<<"cnt "<<cnt<<std::endl;
    std::cout<<"send commander over"<<std::endl;
    std::string bbuffer = sp.read(1024);
    std::cout<<"149  bbuffer "<<bbuffer<<std::endl;
}
...
public:
    ...
    void callback(const moveit_msgs::msg::DisplayTrajectory::ConstPtr &msg)
    {
        ...
        for (int i = 1; i < n - 1; i++)
        {
            ...
            std::string bbuffer = sp.read(1024);
            std::cout<<"bbuffer "<<bbuffer<<std::endl;
...
int main(int argc, char** argv)
{
    ...
    if(sp.isOpen())
        RCLCPP_INFO_STREAM(node->get_logger(), "/dev/ttyUSB0 is opened.");
    else
        return -1;

    RCLCPP_INFO_STREAM(node->get_logger(), "351");
//使能
    for (int i = 0; i < 3; i++)
    {
        ...
        RCLCPP_INFO_STREAM(node->get_logger(), "359");
    }
    ...
    RCLCPP_INFO_STREAM(node->get_logger(), "364");
